...
export class CSFDatabase {
    ...
    initializeSchema() {
        try {
            ...
            const tableExists = this.db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='organization_profiles'").get();
            if (!tableExists) {
    ...
    getImplementation(orgId, subcategoryId) {
        const stmt = this.db.prepare('SELECT * FROM subcategory_implementations WHERE org_id = ? AND subcategory_id = ?');
        const result = stmt.get(orgId, subcategoryId);
        if (!result) {
            return undefined;
        }
        return {
    ...
    `);
        stmt.run(impl.org_id, impl.subcategory_id, impl.implementation_status, impl.maturity_level, impl.notes, evidenceJson, impl.assessed_by);
    }
    // ============================================================================
    ...
    getRiskAssessment(orgId, elementId) {
        const stmt = this.db.prepare('SELECT * FROM risk_assessments WHERE org_id = ? AND element_id = ?');
        return stmt.get(orgId, elementId);
    }
    upsertRiskAssessment(risk) {
        const stmt = this.db.prepare(`
    ...
    `);
        ...
        stmt.run(risk.org_id, risk.element_id, risk.risk_level, risk.likelihood, risk.impact, riskScore, risk.mitigation_status, risk.mitigation_plan, risk.next_review_date);
    }
    // ============================================================================
    // GAP ANALYSIS
    // ============================================================================
    getGapAnalyses(orgId) {
        const stmt = this.db.prepare('SELECT * FROM gap_analysis WHERE org_id = ?');
        return stmt.all(orgId);
    }
    getGapAnalysis(orgId, categoryId) {
        const stmt = this.db.prepare('SELECT * FROM gap_analysis WHERE org_id = ? AND category_id = ?');
        return stmt.get(orgId, categoryId);
    }
    upsertGapAnalysis(gap) {
    ...
    `);
        stmt.run(gap.org_id, gap.category_id, gap.current_score, gap.target_score, gapScore, gap.priority, gap.estimated_effort, gap.target_date);
    }
    // ============================================================================
    ...
    `);
        stmt.run(profile.profile_id, profile.org_id, profile.profile_name, profile.profile_type, profile.description || null, profile.created_by || null, profile.parent_profile_id || null);
    }
    /**
    ...
    `);
        stmt.run(workflow.workflow_id, workflow.profile_id, workflow.org_id, workflow.state, workflow.assessment_scope, workflow.target_functions ? JSON.stringify(workflow.target_functions) : null, workflow.timeline_weeks, workflow.started_at, workflow.contact_name, workflow.contact_email);
    }
    /**
     * Get assessment workflow by workflow ID
     */
    getAssessmentWorkflow(workflowId) {
     ...
     */
    getAnsweredQuestionsCount(profileId) {
     ...
     */
    validateAssessmentDataAuthenticity(profileId) {
        try {
    ...
    getOrganizationProfiles(orgId) {
        const stmt = this.db.prepare('SELECT * FROM profiles WHERE org_id = ? AND is_active = 1');
        return stmt.all(orgId);
    }
    cloneProfile(sourceProfileId, newProfileId, newName) {
        // Clone the profile
    ...
    `);
        stmt.run(assessment.profile_id, assessment.subcategory_id, assessment.implementation_level, assessment.maturity_score, assessment.confidence_level || 'medium', assessment.notes || null, assessment.evidence || null, assessment.assessed_by || null);
    }
    createBulkAssessments(profileId, assessments) {
        const stmt = this.db.prepare(`
      INSERT INTO assessments (
        profile_id, subcategory_id, implementation_level,
        maturity_score, confidence_level, notes, evidence, assessed_by
      ...
      ON CONFLICT(profile_id, subcategory_id) DO UPDATE SET
        ...
        assessed_at = CURRENT_TIMESTAMP
    `);
        const insertMany = this.db.transaction((assessments) => {
            for (const assessment of assessments) {
                stmt.run(profileId, assessment.subcategory_id, assessment.implementation_level, assessment.maturity_score, assessment.confidence_level || 'medium', assessment.notes || null, assessment.evidence || null, assessment.assessed_by || null);
            }
        });
        insertMany(assessments);
    }
    getProfileAssessments(profileId) {
        const stmt = this.db.prepare('SELECT * FROM assessments WHERE profile_id = ?');
        return stmt.all(profileId);
    }
    getAssessmentsBySubcategory(profileId, subcategoryId) {
        const stmt = this.db.prepare('SELECT * FROM assessments WHERE profile_id = ? AND subcategory_id = ?');
        return stmt.get(profileId, subcategoryId);
    }
    // ============================================================================
    // GAP ANALYSIS OPERATIONS
    // ============================================================================
    generateGapAnalysis(currentProfileId, targetProfileId, analysisId) {
    ...
    `;
        // Execute the insert
        this.db.prepare(sql).run(currentProfileId, targetProfileId, analysisId, currentProfileId);
    ...
    getGapAnalysisDetails(analysisId) {
        const sql = `
      SELECT 
        g.*,
        p.org_name,
    ...
    getPriorityMatrixFromAssessments(profileId) {
        const stmt = this.db.prepare(`
      SELECT 
        a.subcategory_id,
    ...
    // ============================================================================
    getMaturityByFunction(profileId) {
    ...
    `;
      ...
      WHERE profile_id = ?
    `;
        const overall = this.db.prepare(overallSql).get(profileId);
        return {
            overall_risk_score: overall?.overall_risk_score || 0,
            function_risks: functionRisks
        };
    }
    getMaturityTrend(profileId, startDate, endDate) {
    ...
    `;
        return this.db.prepare(sql).all(profileId, endDate, profileId, startDate, endDate);
    }
    getComprehensiveMaturityAnalysis(profileId) {
    ...
    `);
        planStmt.run(planId, plan.gap_analysis_id, plan.profile_id, plan.plan_name, plan.timeline_months, plan.available_resources, plan.total_phases, plan.total_effort_hours, plan.estimated_cost, plan.status || 'draft');
        return planId;
    }
    createImplementationPhase(phase) {
        const stmt = this.db.prepare(`
    ...
    `);
        stmt.run(phase.id, phase.plan_id, phase.phase_number, phase.phase_name, phase.start_month, phase.end_month, phase.effort_hours, phase.resource_count, phase.status || 'pending');
    }
    createImplementationItem(item) {
        const stmt = this.db.prepare(`
    ...
    `);
        stmt.run(item.id, item.phase_id, item.subcategory_id, item.priority_rank, item.effort_hours, item.dependencies, item.status || 'pending', item.completion_percentage || 0);
    }
    getImplementationPlan(planId) {
    ...
    getSubcategoryDependencies(subcategoryId) {
    ...
    createSubcategoryDependency(dep) {
        const stmt = this.db.prepare(`
    ...
    `);
        stmt.run(dep.id, dep.subcategory_id, dep.depends_on_subcategory_id, dep.dependency_type, dep.dependency_strength || 5);
    }
    getCostEstimate(subcategoryId, organizationSize) {
        return this.db.prepare(`
      SELECT * FROM cost_estimates 
      WHERE subcategory_id = ? AND organization_size = ?
    `).get(subcategoryId, organizationSize);
    }
    createCostEstimate(estimate) {
        const stmt = this.db.prepare(`
    ...
    `);
        stmt.run(estimate.id, estimate.subcategory_id, estimate.organization_size, estimate.labor_cost, estimate.tools_cost, estimate.training_cost, estimate.total_cost, estimate.effort_hours);
    }
    getSuggestedActions(profileId, capacityHours) {
        const sql = `
    ...
    getGapAnalysisItems(gapAnalysisId) {
        return this.db.prepare(`
      SELECT 
        g.*,
        s.name as subcategory_name,
    ...
    calculateDependencyGraph(subcategoryIds) {
    ...
    // ============================================================================
    upsertProgressTracking(progress) {
        const stmt = this.db.prepare(`
    ...
    createProgressMilestone(milestone) {
        const stmt = this.db.prepare(`
    ...
    `);
        stmt.run(milestone.id, milestone.profile_id, milestone.milestone_name, milestone.target_date, milestone.completion_date, milestone.status || 'pending', milestone.completion_percentage || 0, milestone.subcategories_involved, milestone.success_criteria);
    }
    getProgressMilestones(profileId) {
        return this.db.prepare(`
      SELECT 
        *,
        CASE 
    ...
    calculateVelocity(profileId, daysBack = 30) {
        const sql = `
    ...
    // ============================================================================
    upsertIndustryBenchmark(benchmark) {
        const stmt = this.db.prepare(`
      ...
      ON CONFLICT(industry, organization_size, csf_function, metric_name, data_year) DO UPDATE SET
        ...
        notes = excluded.notes
    `);
        stmt.run(benchmark.id || `${benchmark.industry}_${benchmark.organization_size}_${benchmark.csf_function}_${benchmark.metric_name}_${benchmark.data_year}`, benchmark.industry, benchmark.organization_size, benchmark.csf_function, benchmark.metric_name, benchmark.percentile_25, benchmark.percentile_50, benchmark.percentile_75, benchmark.percentile_90, benchmark.average_score, benchmark.sample_size, benchmark.data_year, benchmark.source, benchmark.notes);
    }
    getIndustryBenchmarks(industry, organizationSize) {
        return this.db.prepare(`
    ...
    compareProfileToBenchmark(profileId, industry, organizationSize) {
        const sql = `
    ...
    // ============================================================================
    getExecutiveReportData(profileId) {
        const sql = `
    ...
    getTechnicalReportData(profileId) {
        const sql = `
    ...
    getProgressReportData(profileId) {
        const sql = `
    ...
    importAssessmentBatch(profileId, assessments, conflictMode = 'overwrite') {
        const result = {
            imported: 0,
            skipped: 0,
            errors: []
        ...
        const transaction = this.db.transaction(() => {
            for (const assessment of assessments) {
                try {
                    ...
                    const subcategoryExists = this.db.prepare('SELECT subcategory_id FROM csf_subcategories WHERE subcategory_id = ?').get(assessment.subcategory_id);
          ...
          `).get(profileId, assessment.subcategory_id);
              ...
              SET implementation_level = ?,
                  ...
                  assessed_by = ?
              WHERE assessment_id = ?
            `).run(assessment.implementation_level, mergedScore, mergedNotes, assessment.assessed_by || 'imported', existing.assessment_id);
                        result.imported++;
                    }
              ...
              SET implementation_level = ?,
                  ...
                  assessed_by = ?
              WHERE profile_id = ? AND subcategory_id = ?
            `).run(assessment.implementation_level, assessment.maturity_score, assessment.notes, assessment.assessed_by || 'imported', profileId, assessment.subcategory_id);
                        result.imported++;
                    }
                    else {
              ...
              VALUES (?, ?, ?, ?, ?, ?, ?)
            `).run(assessmentId, profileId, assessment.subcategory_id, assessment.implementation_level, assessment.maturity_score, assessment.notes, assessment.assessed_by || 'imported');
                        result.imported++;
                    }
                }
                catch (error) {
    ...
    addAuditEvidence(evidence) {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(evidenceId, evidence.assessment_id, evidence.profile_id, evidence.subcategory_id, evidence.file_name, evidence.file_type, evidence.file_size, evidence.file_hash, evidence.uploaded_by || 'system', evidence.evidence_type, evidence.description, evidence.tags ? JSON.stringify(evidence.tags) : null, evidence.metadata ? JSON.stringify(evidence.metadata) : null);
        return evidenceId;
    }
    validateEvidence(evidenceId, status, notes, validatedBy) {
        this.db.prepare(`
    ...
    getEvidenceForAssessment(assessmentId) {
        return this.db.prepare(`
    ...
    getEvidenceValidationReport(profileId) {
    ...
    getImportValidationErrors(data, _format) {
        const errors = [];
        const validSubcategories = new Set(this.db.prepare('SELECT subcategory_id FROM csf_subcategories').all()
     ...
     */
    seedQuestionBankData(subcategoryId, questionData) {
        const transaction = this.db.transaction(() => {
            // Insert questions
      ...
      `);
            for (const question of questionData.questions) {
                insertQuestion.run(question.id, subcategoryId, question.questionText, question.questionType, question.helpText || null, question.organizationSize || null, question.sector || null, question.weight || 1.0, question.required ? 1 : 0);
                // Insert options if provided
                if (question.options) {
                    for (const option of question.options) {
                        insertOption.run(option.id, question.id, option.value, option.label, option.description || null, option.weight || 1.0, option.sortOrder || 0);
                    }
                }
                // Insert examples if provided
                if (question.examples) {
                    for (const example of question.examples) {
                        insertExample.run(example.id, question.id, example.text, example.type || 'implementation', example.organizationSize || null, example.sector || null, example.sortOrder || 0);
                    }
                }
            }
     ...
     */
    getQuestionsBySubcategory(subcategoryId, filters) {
        let sql = `
      SELECT 
        qb.*,
        s.name as subcategory_name,
     ...
     */
    getQuestionExamples(questionId, filters) {
        let sql = `
     ...
     */
    getQuestionContext(subcategoryId) {
    ...
    `);
        stmt.run(response.id, response.profileId, response.questionId, response.subcategoryId, response.responseValue || null, response.responseText || null, response.confidenceLevel || null, response.notes || null, JSON.stringify(response.evidence || []), response.respondedBy || null);
    }
    /**
     * Get question responses for a profile
     */
    getQuestionResponses(profileId, subcategoryId) {
        let sql = `
      SELECT 
        qr.*,
        qb.question_text,
