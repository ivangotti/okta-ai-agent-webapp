 ...
 */
export async function validateAssessmentResponses(params, db = getDatabase()) {
    ...
    try {
        ...
        validationResults.completeness_percentage = Math.round((answeredSubcategories.size / expectedQuestions.length) * 100);
 ...
 */
function validateResponseValue(response, question) {
 ...
 */
function isCriticalResponse(response, _question) {
 ...
 */
async function checkConsistencyWithPrevious(profileId, responses, db) {
    const warnings = [];
    try {
    ...
    `).all(profileId);
        ...
        for (const response of responses) {
            const previous = previousAssessments.find((p) => p.subcategory_id === response.subcategory_id);
 ...
 */
async function getProfile(profileId, db) {
    try {
