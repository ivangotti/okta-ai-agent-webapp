...
export async function generateGapAnalysis(params) {
    ...
    try {
        ...
        const analysisResult = db.generateGapAnalysis(params.current_profile_id, params.target_profile_id, analysisId);
        ...
        const filteredGaps = gapDetails.filter((gap) => gap.gap_score >= params.minimum_gap_score);
        ...
        const recommendations = generateRecommendations(filteredGaps, priorityMatrix, gapSummary);
 ...
 */
function generatePriorityMatrix(gaps) {
    const matrix = {
        'Quick Win': {
            quadrant: 'Quick Win',
            items: [],
    ...
    result.sort((a, b) => priorityOrder.indexOf(a.quadrant) - priorityOrder.indexOf(b.quadrant));
    return result;
}
/**
 * Generate visualizations data
 */
function generateVisualizations(gaps, currentProfile, targetProfile) {
    // Gap by function
    const gapByFunction = {};
    for (const gap of gaps) {
    ...
    for (const func of Object.values(gapByFunction)) {
        func.average_gap_score =
            func.gaps.reduce((sum, g) => sum + g, 0) / func.gaps.length;
 ...
 */
function generateRecommendations(gaps, priorityMatrix, gapSummary) {
    ...
    if (quickWins && quickWins.items.length > 0) {
        ...
        for (const item of topQuickWins) {
            recommendations.immediate_actions.push(`Implement ${item.subcategory_id}: ${item.improvement_required} (Gap: ${item.gap_score}%)`);
        }
    }
    ...
    for (const gap of criticalGaps) {
        if (!recommendations.immediate_actions.some(a => a.includes(gap.subcategory_id))) {
            recommendations.immediate_actions.push(`Address critical gap in ${gap.subcategory_id} (${gap.gap_score}% gap)`);
        }
    }
    ...
    if (strategic && strategic.items.length > 0) {
        ...
        for (const item of topStrategic) {
            recommendations.short_term_goals.push(`Plan implementation of ${item.subcategory_id}: ${item.improvement_required}`);
        }
    }
    ...
    for (const [funcId, totalGap] of sortedFunctions) {
        ...
        recommendations.long_term_objectives.push(`Strengthen ${funcName} function capabilities (Total gap: ${Math.round(totalGap)})`);
    }
    // Resource requirements based on effort scores
    if (gapSummary.total_effort_required > 100) {
        recommendations.resource_requirements.push('Significant resource allocation required (100+ effort points)');
    }
    if (gapSummary.critical_gaps > 5) {
        recommendations.resource_requirements.push('Dedicated security team needed for critical gap remediation');
    }
    if (quickWins && quickWins.item_count > 10) {
        recommendations.resource_requirements.push(`${quickWins.item_count} quick wins available - allocate resources for rapid implementation`);
    }
    if (strategic && strategic.total_effort > 50) {
        recommendations.resource_requirements.push('Multi-phase project planning required for strategic initiatives');
    }
 ...
 */
function calculateProfileSummary(profileId, db) {
    const assessments = db.getProfileAssessments(profileId);
    if (!assessments || assessments.length === 0) {
        return { maturity: 0, implementation: 0 };
    }
    const implemented = assessments.filter((a) => a.implementation_level !== 'not_implemented').length;
 ...
 */
function describeImprovement(gap) {
