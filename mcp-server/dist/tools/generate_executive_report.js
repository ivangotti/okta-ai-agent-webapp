...
function validateParams(params) {
    ...
    if (!params.report_period)
        errors.push('report_period is required');
...
async function generateExecutiveReport(params, db) {
    try {
        // Validate input
...
function generateExecutiveSummary(assessments, _reportPeriod) {
    ...
    if (maturityScore >= 90)
        ...
        overallPosture = 'adequate';
    else if (maturityScore >= 50)
        overallPosture = 'needs_improvement';
    else
...
function generateBudgetImpactSummary(posture, criticalGaps) {
...
function generateSecurityMetrics(assessments, comparisonPeriod) {
    const totalMaturity = assessments.reduce((sum, a) => sum + (a.maturity_score || 0), 0);
    ...
    const implemented = assessments.filter(a => a.implementation_level === 'fully_implemented' || a.implementation_level === 'largely_implemented').length;
...
function generateRiskAnalysis(assessments) {
    ...
    return {
        ...
        top_risks: topRisks,
        risk_appetite_alignment: 'under_review',
        emerging_threats: [
...
function generateComplianceOverview(assessments) {
    const implementedPercentage = Math.round((assessments.filter(a => a.implementation_level === 'fully_implemented').length / assessments.length) * 100);
    return {
        primary_frameworks: [
            {
            ...
            },
            {
                ...
                audit_readiness: 'needs_preparation',
...
function generateFinancialImpact(assessments, budgetContext) {
...
function generateIndustryComparison(_assessments, profile) {
...
function generateStrategicRecommendations(_assessments, audience) {
...
function generateImplementationRoadmap(_assessments, _reportPeriod) {
...
export const generateExecutiveReportTool = {
