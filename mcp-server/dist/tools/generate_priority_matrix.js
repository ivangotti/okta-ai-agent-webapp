 ...
 */
export async function generatePriorityMatrix(params) {
    ...
    try {
        ...
        const processedItems = processMatrixItems(matrixData, params.matrix_type, framework);
        // Classify items into quadrants
        const quadrants = classifyIntoQuadrants(processedItems, params.matrix_type, params.max_items_per_quadrant);
        ...
        const config = MATRIX_CONFIGS[params.matrix_type];
        return {
            success: true,
 ...
 */
function processMatrixItems(data, matrixType, _framework) {
    const items = [];
 ...
 * Classify items into quadrants
 */
function classifyIntoQuadrants(items, matrixType, maxItemsPerQuadrant) {
    const config = MATRIX_CONFIGS[matrixType];
    const xThreshold = config.x_axis_threshold;
    const yThreshold = config.y_axis_threshold;
    const quadrants = {
        high_value_low_effort: createQuadrant(config.quadrant_names.high_value_low_effort, 'High value with low effort - prioritize for immediate implementation', 'Allocate resources immediately for quick implementation'),
        high_value_high_effort: createQuadrant(config.quadrant_names.high_value_high_effort, 'High value but requires significant effort - plan strategically', 'Develop phased implementation plan with milestones'),
        low_value_low_effort: createQuadrant(config.quadrant_names.low_value_low_effort, 'Low value but easy to implement - consider as fill-in work', 'Implement during available capacity or downtime'),
        low_value_high_effort: createQuadrant(config.quadrant_names.low_value_high_effort, 'Low value and high effort - generally avoid or deprioritize', 'Defer or reconsider necessity')
    };
    // Classify each item
 ...
 * Calculate summary statistics
 */
function calculateSummary(quadrants) {
    const allItems = [];
    for (const quadrant of Object.values(quadrants)) {
        allItems.push(...quadrant.items);
    }
    return {
 ...
 */
function generateRecommendations(quadrants, summary, matrixType) {
    ...
    if (quickWins.items.length > 0) {
        ...
        for (const item of topItems) {
            recommendations.immediate_focus.push(`Implement ${item.subcategory_id}: Low effort (${item.x_axis_value}/10), High impact (${item.y_axis_value}/10)`);
        }
    }
    // Resource allocation
    if (summary.quick_wins_count > 5) {
        recommendations.resource_allocation.push(`Allocate dedicated team for ${summary.quick_wins_count} quick wins (${quickWins.total_effort_hours} hours estimated)`);
    }
    if (summary.strategic_initiatives_count > 0) {
        recommendations.resource_allocation.push(`Plan resources for ${summary.strategic_initiatives_count} strategic initiatives (${quadrants.high_value_high_effort.total_effort_hours} hours)`);
    }
    // Phased approach
    if (summary.total_items > 10) {
        recommendations.phased_approach.push(`Phase 1 (0-3 months): Focus on ${summary.quick_wins_count} quick wins`);
        recommendations.phased_approach.push(`Phase 2 (3-9 months): Tackle ${Math.min(3, summary.strategic_initiatives_count)} strategic initiatives`);
        recommendations.phased_approach.push(`Phase 3 (9-12 months): Address ${summary.fill_ins_count} fill-in items during available capacity`);
    }
    ...
    if (matrixType === 'risk_feasibility' || matrixType === 'effort_impact') {
        ...
        if (highRiskItems.length > 0) {
            recommendations.risk_mitigation.push(`${highRiskItems.length} items provide significant risk reduction (>70%)`);
        }
    }
    // Add specific recommendations based on distribution
    if (summary.quick_wins_count === 0 && summary.strategic_initiatives_count > 5) {
        recommendations.risk_mitigation.push('No quick wins identified - consider breaking down strategic initiatives into smaller tasks');
    }
    if (summary.avoid_count > summary.quick_wins_count) {
        recommendations.risk_mitigation.push('High number of low-value/high-effort items - review requirements and consider alternatives');
    }
 ...
 */
function generateResourceEstimates(summary, quadrants) {
    ...
    const skillRequirements = new Set();
...
function estimateFeasibility(item) {
    const effort = estimateEffort(item);
    return Math.max(1, 11 - effort);
}
function estimateRiskReduction(item) {
    const gap = item.gap_score || 50;
 ...
 */
function createQuadrant(name, description, strategy) {
    return {
        name,
        description,
        strategy,
