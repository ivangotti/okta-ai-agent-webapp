...
export async function exportData(db, options) {
    try {
        ...
        const includedSections = ['profile'];
        let recordCount = 1;
...
function convertToCSV(data) {
    ...
    if (data.assessments && data.assessments.length > 0) {
        ...
        data.assessments.forEach((a) => {
            csvSections.push(`"${a.subcategory_id}","${a.implementation_level || ''}",${a.maturity_score || 0},"${a.notes || ''}","${a.assessed_at || ''}","${a.assessed_by || ''}"`);
        });
        csvSections.push('');
    }
    // Progress Tracking
    if (data.progress_tracking && data.progress_tracking.length > 0) {
        ...
        data.progress_tracking.forEach((p) => {
            csvSections.push(`"${p.subcategory_id}","${p.current_implementation || ''}",${p.current_maturity || 0},${p.completion_percentage || 0},"${p.status || ''}",${p.is_blocked ? 'Yes' : 'No'},"${p.last_updated || ''}"`);
        });
        csvSections.push('');
    }
    // Compliance Mappings
    if (data.compliance_mappings && data.compliance_mappings.length > 0) {
        ...
        data.compliance_mappings.forEach((c) => {
            csvSections.push(`"${c.framework}","${c.control_id}","${c.control_name || ''}","${c.csf_subcategory_id}","${c.mapping_strength || ''}",${c.coverage_percentage || 0}`);
        });
        csvSections.push('');
    }
    // Milestones
    if (data.milestones && data.milestones.length > 0) {
        ...
        data.milestones.forEach((m) => {
            csvSections.push(`"${m.milestone_name}","${m.target_date || ''}","${m.completion_date || ''}","${m.status || ''}",${m.completion_percentage || 0}`);
        });
    }
