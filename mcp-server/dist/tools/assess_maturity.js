 ...
 */
export async function assessMaturity(params) {
    ...
    try {
        ...
        if (!assessmentDataValidation.isAuthentic) {
            return createErrorResult(params.profile_id, `Cannot perform maturity assessment: ${assessmentDataValidation.reason}. ` +
        ...
        const functionBreakdown = processFunctionMaturity(functionMaturityData, comprehensiveAnalysis, params.include_recommendations, params.include_subcategory_details);
 ...
 */
function processFunctionMaturity(functionData, comprehensiveData, includeRecommendations, includeDetails) {
    const results = [];
    ...
    for (const func of functionData) {
        ...
        if (includeRecommendations) {
            functionMaturity.recommendations = generateFunctionRecommendations(func.function_id, func.maturity_tier, func.implementation_percentage);
        }
 ...
 * Calculate overall metrics
 */
function calculateOverallMetrics(functionBreakdown) {
    ...
    if (overallPercentage > 75)
        overallTier = 'Adaptive';
    else if (overallPercentage > 50)
        overallTier = 'Repeatable';
    else if (overallPercentage > 25)
        overallTier = 'Risk-Informed';
 ...
 */
function generateRecommendations(functionBreakdown, overallMetrics) {
    ...
    for (const func of weakFunctions) {
        immediatePriorities.push(`Urgent: Improve ${func.function_name} function (currently at ${func.implementation_percentage.toFixed(1)}%)`);
    }
    // Identify strategic improvements for mid-tier functions
    const midFunctions = functionBreakdown.filter(f => f.maturity_tier === 'Risk-Informed' || f.maturity_tier === 'Repeatable');
    for (const func of midFunctions) {
        if (func.max_maturity_score - func.min_maturity_score > 2) {
            strategicImprovements.push(`Standardize ${func.function_name} practices to reduce variation (current range: ${func.min_maturity_score}-${func.max_maturity_score})`);
        }
    }
    // Identify quick wins
    for (const func of functionBreakdown) {
        if (func.implementation_percentage >= 60 && func.implementation_percentage < 75) {
            quickWins.push(`${func.function_name} is close to next tier (${func.implementation_percentage.toFixed(1)}%) - small improvements will yield tier advancement`);
        }
    }
    // Add overall recommendations based on tier
    const tierRecs = MATURITY_TIERS[overallMetrics.overall_tier];
    if (tierRecs) {
 ...
 */
function generateFunctionRecommendations(functionId, tier, percentage) {
