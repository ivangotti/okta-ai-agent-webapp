...
function validateParams(params) {
    ...
    if (!params.compliance_framework)
        errors.push('compliance_framework is required');
...
async function generateComplianceReport(params, db) {
    try {
        // Validate input
...
function getFrameworkRequirements(framework, customRequirements) {
...
function analyzeCompliance(requirements, assessments) {
    return requirements.map(requirement => {
        const mappedSubcategories = requirement.csf_mappings || [];
        const relevantAssessments = assessments.filter(a => mappedSubcategories.includes(a.subcategory_id));
...
function calculateOverallScore(complianceDetails) {
    if (complianceDetails.length === 0)
        return 0;
    const scores = complianceDetails.map((detail) => {
...
function generateExecutiveSummary(complianceDetails) {
...
function generateGapAnalysis(complianceDetails) {
...
function generateRecommendations(complianceDetails, _framework) {
...
function generateRiskAssessment(complianceDetails, _framework) {
    ...
    const nonComplianceRisks = nonCompliantItems.map(detail => ({
        ...
        likelihood: detail.remediation_priority === 'critical' ? 'high' : 'medium',
        impact: detail.risk_level,
...
function generateImplementationRoadmap(_complianceDetails, _framework) {
    const now = new Date();
