 ...
 */
export async function searchFramework(params) {
    try {
        ...
        const elements = params.element_types && params.element_types.length > 0
            ? getElementsOfTypes(framework, params.element_types)
 ...
 */
function scoreElement(element, searchTerms, fuzzy) {
    const matches = [];
    let totalScore = 0;
 ...
 */
function scoreField(field, searchTerms, fuzzy) {
 ...
 */
function getSnippet(text, searchTerms, maxLength = 150) {
 ...
 */
function getAllElements(framework) {
 ...
 */
function getElementsOfTypes(framework, types) {
    const elements = [];
 ...
 */
function getStatsByType(results) {
    const stats = {};
    for (const result of results) {
        const type = result.element.element_type;
