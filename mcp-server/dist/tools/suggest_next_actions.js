...
export async function suggestNextActions(params) {
    ...
    try {
        ...
        if (params.focus_area !== 'all') {
            ...
            filteredSuggestions = dbSuggestions.filter((s) => s.function_id === focusFunctionId);
        }
        ...
        for (const suggestion of filteredSuggestions) {
            ...
            const dependencies = params.include_dependencies
                ...
                : { blocking: [], recommended: [], status: 'Ready' };
        ...
        const dependenciesToAddress = params.include_dependencies
            ...
            : undefined;
        // Create timeline
        const timeline = createTimeline(suggestedActions, params.capacity_hours_per_week, params.time_horizon_weeks);
        // Generate recommendations
        const recommendations = generateRecommendations(suggestedActions, capacityAnalysis, params);
        return {
            success: true,
 ...
 */
async function analyzeDependencies(subcategoryId, profileId, db, _framework) {
    ...
    const implementedMap = new Map(assessments.map((a) => [a.subcategory_id, a.implementation_level]));
    const blocking = [];
    const recommended = [];
    ...
    for (const depFunc of functionDeps) {
        ...
        const funcImplemented = assessments.some((a) => a.subcategory_id.startsWith(depFunc) &&
 ...
 */
function calculateImpact(suggestion, optimizationGoal) {
    const priorities = FUNCTION_PRIORITIES[optimizationGoal];
 ...
 */
function generateJustification(suggestion, dependencies, impact, optimizationGoal) {
 ...
 */
function generateImplementationTips(suggestion, framework) {
    const tips = [];
    ...
    if (subcategory && 'implementation_examples' in subcategory &&
        Array.isArray(subcategory.implementation_examples) &&
        subcategory.implementation_examples.length > 0) {
        tips.push(`Reference: ${subcategory.implementation_examples[0]}`);
    }
 ...
 */
function identifyBlockingDependencies(actions, _db) {
 ...
 */
function createTimeline(actions, weeklyCapacity, horizonWeeks) {
    const timeline = [];
    let currentWeek = 1;
    let weekHours = 0;
    let weekActions = [];
 ...
 */
function generateWeeklyMilestones(actions, week) {
    const milestones = [];
    if (week === 1) {
 ...
 * Generate recommendations
 */
function generateRecommendations(actions, capacity, params) {
    ...
    if (readyActions.length > 0) {
        ...
        if (firstAction) {
            recommendations.immediate_priorities.push(`Start with ${firstAction.subcategory_id}: ${firstAction.subcategory_name}`);
            if (firstAction.estimated_effort.complexity === 'Low') {
                recommendations.immediate_priorities.push('Low complexity - ideal for building momentum');
            }
        }
    }
    // Resource optimization
    if (capacity.utilization_percentage > 90) {
        recommendations.resource_optimization.push('High utilization - consider increasing capacity or extending timeline');
    }
    else if (capacity.utilization_percentage < 60) {
        recommendations.resource_optimization.push('Capacity available - consider accelerating implementation');
    }
    if (capacity.remaining_hours > 20) {
        recommendations.resource_optimization.push(`${capacity.remaining_hours}h available for additional quick wins`);
    }
    // Dependency management
    const blockedActions = actions.filter(a => a.dependencies.status === 'Blocked');
    if (blockedActions.length > 0) {
        recommendations.dependency_management.push(`Address dependencies to unblock ${blockedActions.length} actions`);
    }
    // Success factors
    recommendations.success_factors.push('Maintain regular progress tracking', 'Adjust priorities based on learnings');
    if (params.optimization_goal === 'quick_wins') {
        recommendations.success_factors.push('Celebrate early wins to build momentum');
    }
    else if (params.optimization_goal === 'risk_reduction') {
        recommendations.success_factors.push('Focus on highest risk areas first');
    }
    return recommendations;
}
// Helper functions
function getFunctionId(focusArea) {
    const mapping = {
        'govern': 'GV',
        'identify': 'ID',
        'protect': 'PR',
...
function getComplexity(implementationLevel) {
