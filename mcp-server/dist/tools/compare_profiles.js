...
export async function compareProfiles(db, profile_ids) {
    try {
        // Validate input
        ...
        // Parse the JSON data
        const profiles = JSON.parse(comparisonData.profiles || '[]');
        const functionScores = JSON.parse(comparisonData.function_scores || '[]');
        ...
        const recommendations = generateComparisonRecommendations(profiles, scoreMatrix, strengths, weaknesses, variance);
...
function identifyCommonPatterns(scoreMatrix, functionNames) {
    const strengths = [];
    const weaknesses = [];
...
function calculateCorrelation(scoreMatrix) {
...
function generateComparisonRecommendations(profiles, scoreMatrix, _strengths, weaknesses, variance) {
    ...
    const topPerformer = profiles.reduce((prev, current) => (current.avg_maturity > prev.avg_maturity) ? current : prev);
    if (topPerformer) {
