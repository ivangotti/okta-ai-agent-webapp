...
export async function validateEvidence(db, options) {
    const result = {
        success: true,
        evidence_ids: [],
        validation_report: {
    ...
    try {
        ...
        for (const evidenceFile of options.evidence_files) {
            ...
            try {
                ...
                if (options.auto_validate) {
                    const validationStatus = performAutoValidation(evidenceType, fileSize, fileExt);
                    db.validateEvidence(evidenceId, validationStatus.status, validationStatus.notes, 'auto-validator');
                ...
                updateCompletenessCheck(result.validation_report.completeness_check, evidenceType);
            }
            catch (error) {
        ...
        result.validation_report.recommendations = generateRecommendations(result.validation_report.completeness_check, result.validation_report.file_details);
...
function inferEvidenceType(extension) {
    const ext = extension.toLowerCase();
    if (ALLOWED_FILE_TYPES.screenshot?.includes(ext))
        return 'screenshot';
    if (ALLOWED_FILE_TYPES.document?.includes(ext))
        return 'document';
    if (ALLOWED_FILE_TYPES.log?.includes(ext))
        return 'log';
    if (ALLOWED_FILE_TYPES.report?.includes(ext))
        return 'report';
    if (ALLOWED_FILE_TYPES.config?.includes(ext))
        return 'config';
    return 'other';
}
function isFileTypeAllowed(extension, evidenceType) {
    const allowedTypes = ALLOWED_FILE_TYPES[evidenceType];
    if (!allowedTypes)
        return false;
    if (allowedTypes.length === 0)
        return true; // 'other' type allows all
    return allowedTypes.includes(extension.toLowerCase());
}
function performAutoValidation(evidenceType, fileSize, fileExt) {
    const issues = [];
...
function updateCompletenessCheck(completeness, evidenceType) {
    switch (evidenceType) {
        case 'document':
        case 'report':
...
function generateRecommendations(completeness, fileDetails) {
