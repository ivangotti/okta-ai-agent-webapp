...
Example:
{
  ...
  "confidence_level": "medium"
        ...
        const assessments = generateAssessments(framework, params.simplified_answers, params.confidence_level, params.assessed_by, params.notes);
 ...
 */
function generateAssessments(framework, answers, confidenceLevel, assessedBy, notes) {
    const assessments = [];
    ...
    for (const subcategory of allSubcategories) {
        ...
        if (!functionKey || !answers[functionKey]) {
            continue;
        }
        const answer = answers[functionKey];
 ...
 */
function getFunctionKey(functionId) {
    const reverseMap = {};
 ...
 */
function getSubcategoryVariance(subcategoryId) {
 ...
 */
function calculateFunctionSummaries(assessments) {
 ...
 */
function calculateMaturityScores(answers) {
    const scores = {};
    let total = 0;
