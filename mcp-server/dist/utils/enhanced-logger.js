 ...
 */
const structuredFormat = winston.format.combine(winston.format.timestamp({ format: 'YYYY-MM-DD HH:mm:ss.SSS' }), winston.format.errors({ stack: true }), winston.format.printf((info) => {
 ...
 */
const consoleFormat = winston.format.combine(winston.format.colorize(), winston.format.timestamp({ format: 'HH:mm:ss.SSS' }), winston.format.printf(({ timestamp, level, message, correlationId, ...meta }) => {
    const context = asyncLocalStorage.getStore() || {};
...
class EnhancedLogger {
     ...
     * Log database query
     */
    logQuery(query, duration, success, meta) {
     ...
     * Log tool execution
     */
    logToolExecution(toolName, duration, success, meta) {
     ...
     * Log metric
     */
    logMetric(metricName, value, unit, meta) {
        this.info('Metric', {
            metricName,
            value,
            unit,
