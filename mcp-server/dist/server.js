...
import { monitoringMiddleware, errorMonitoringMiddleware, createMonitoringRouter, toolMonitoring } from './middleware/monitoring.js';
...
function createExpressApp() {
    ...
    // MCP tool endpoint
    app.post('/tools/:toolName', async (req, res, _next) => {
        ...
        try {
            ...
            if (ENABLE_AUTH && toolConfig.requiresAuth) {
                ...
                if (toolConfig.requiredScope) {
                    await new Promise((resolve, reject) => {
                        authMiddleware.requireScope(toolConfig.requiredScope)(req, res, (err) => {
                            if (err)
                                reject(err);
                            else
                                resolve();
            ...
            const clientId = req.auth?.client_id || req.ip;
