...
export class FrameworkSeeder {
    ...
    async seedBasicFramework() {
        ...
        try {
            ...
            logger.error('Failed to seed framework structure:', error);
            throw error;
        }
    }
    async seedFunctions() {
    ...
    frameworkDataExists() {
        try {
            ...
            const subcategoryCount = this.db.prepare('SELECT COUNT(*) as count FROM subcategories').get();
            ...
            for (const subcategoryId of requiredSubcategories) {
                const result = this.db.prepare('SELECT COUNT(*) as count FROM subcategories WHERE id = ?').get(subcategoryId);
                if (result.count === 0) {
