...
async function validateIdJagToken(token, expectedAudience) {
    try {
...
function validateIdJagMiddleware(req, res, next) {
...
const HTTP_TOOLS = {
    ...
    'track_progress': { handler: async (params) => trackProgressTool.execute(params, getDatabase()), schema: z.object({
            profile_id: z.string(),
    ...
    'get_industry_benchmarks': { handler: async (params) => getIndustryBenchmarksTool.execute(params, getDatabase()), schema: z.object({
            profile_id: z.string(),
            sector: z.string().optional(),
            organization_size: z.string().optional()
        }) },
    // Reporting Tools
    'generate_report': { handler: async (params) => generateReportTool.execute(params, getDatabase()), schema: z.object({
            profile_id: z.string(),
    ...
    'compare_profiles': { handler: async (params) => compareProfilesTool.execute(params, getDatabase()), schema: z.object({
            profile1_id: z.string(),
            profile2_id: z.string(),
            comparison_type: z.enum(['full', 'maturity', 'implementation', 'risk']).default('full')
        }) },
    'export_data': { handler: async (params) => exportDataTool.execute(params, getDatabase()), schema: z.object({
            profile_id: z.string(),
            export_format: z.enum(['json', 'csv', 'excel']).default('json'),
            include_metadata: z.boolean().default(true)
        }) },
    'import_assessment': { handler: async (params) => importAssessmentTool.execute(params, getDatabase()), schema: z.object({
            profile_id: z.string(),
            assessment_data: z.object({}),
            overwrite_existing: z.boolean().default(false)
        }) },
    // Evidence & Templates
    'validate_evidence': { handler: async (params) => validateEvidenceTool.execute(params, getDatabase()), schema: z.object({
            profile_id: z.string(),
            evidence_id: z.string().optional()
        }) },
    'get_implementation_template': { handler: async (params) => getImplementationTemplateTool.execute(params, getDatabase(), getFrameworkLoader()), schema: z.object({
            subcategory_id: z.string(),
    ...
    'generate_policy_template': { handler: async (params) => generatePolicyTemplateTool.execute(params, getDatabase(), getFrameworkLoader()), schema: z.object({
            subcategory_id: z.string(),
            industry: z.enum(['financial_services', 'healthcare', 'manufacturing', 'technology', 'government', 'retail', 'energy']).optional(),
            organization_size: z.enum(['small', 'medium', 'large', 'enterprise']).optional()
        }) },
    ...
    'get_question_context': { handler: async (params) => getQuestionContext(params), schema: z.object({
            question_id: z.string(),
            subcategory_id: z.string().optional()
        }) },
    // Data Reset
    'reset_organizational_data': { handler: async (params) => resetOrganizationalData(params), schema: z.object({
    ...
    'query_framework': { handler: async (params) => {
    ...
    'get_element': { handler: async (params) => {
    ...
    'get_implementation_examples': { handler: async (params) => {
    ...
    'create_organization': { handler: async (params) => {
            const db = getDatabase();
    ...
    'record_implementation': { handler: async (params) => {
    ...
    'record_risk': { handler: async (params) => {
    ...
    'record_gap': { handler: async (params) => {
    ...
    'get_assessment': { handler: async (params) => {
    ...
    app.post('/api/tools/:toolName', validateIdJagMiddleware, async (req, res) => {
        ...
        try {
            ...
            if (!HTTP_TOOLS[toolName]) {
                return res.status(404).json(createErrorResponse(toolName, `Tool '${toolName}' not found. Available tools: ${Object.keys(HTTP_TOOLS).join(', ')}`));
            }
            const tool = HTTP_TOOLS[toolName];
            // Validate parameters
            let validatedParams;
            try {
                ...
                if (validationError instanceof z.ZodError) {
                    return res.status(400).json(createErrorResponse(toolName, `Invalid parameters: ${validationError.errors.map((e) => `${e.path.join('.')}: ${e.message}`).join(', ')}`));
                }
...
function createSuccessResponse(toolName, data) {
    return {
        success: true,
        data,
        timestamp: new Date().toISOString(),
...
function createErrorResponse(toolName, error) {
    return {
        success: false,
        error,
        timestamp: new Date().toISOString(),
