 ...
 */
export async function cloneProfile(params) {
    ...
    try {
        ...
        const result = db.transaction(() => {
          ...
          WHERE profile_id = ?
        `);
                updateStmt.run(profileType, description, params.modifications?.created_by || sourceProfile.created_by, newProfileId);
            }
            ...
            if (params.modifications?.adjustments && params.modifications.adjustments.length > 0) {
                modificationsApplied = applyAssessmentModifications(db, newProfileId, params.modifications.adjustments);
            }
            return {
 ...
 * Generate new profile ID
 */
function generateNewProfileId(orgId, profileName) {
 ...
 */
function applyAssessmentModifications(db, profileId, adjustments) {
    let modified = 0;
