...
`,
    purpose: (subcategories) => `## 1. Purpose
...
`,
    policy_statement: (requirements) => `## 4. Policy Statement
...
`,
    procedures: (subcategories) => `## 6. Procedures
...
export async function generatePolicyTemplate(_db, framework, options) {
    try {
        ...
        if (options.format === 'structured') {
            templateContent = generateStructuredPolicy(policyName, policyType, subcategories, requirements, complianceMappings, options.include_procedures !== false);
        }
        else {
            // Default to markdown format
            templateContent = generateMarkdownPolicy(policyName, policyType, subcategories, requirements, complianceMappings, options.include_procedures !== false);
        }
        // Build the response
        const template = {
...
function determinePolicyType(subcategories) {
...
function generatePolicyName(subcategories, policyType) {
...
function generatePolicyRequirements(subcategories) {
    const requirements = [];
...
function generateComplianceMappings(subcategories) {
    const mappings = [];
...
function generateMarkdownPolicy(policyName, policyType, subcategories, requirements, complianceMappings, includeProcedures) {
    let content = '';
    // Add sections
...
function generateStructuredPolicy(policyName, policyType, subcategories, requirements, complianceMappings, includeProcedures) {
    const policy = {
        policy_name: policyName,
        policy_type: policyType,
        version: '1.0',
