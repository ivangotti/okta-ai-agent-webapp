...
function validateParams(params) {
    ...
    if (!params.query_mode) {
        ...
        if (!params.performed_by)
            errors.push('performed_by is required');
...
async function trackAuditTrail(params, db) {
    try {
        // Validate input
    ...
    `);
        const insertResult = insertStmt.run(auditId, params.profile_id || null, sanitizedAction, params.resource_type, params.resource_id || null, sanitizedPerformedBy, params.ip_address || null, params.user_agent || null, params.session_id || null, params.authentication_method || null, sanitizedBeforeState ? JSON.stringify(sanitizedBeforeState) : null, sanitizedAfterState ? JSON.stringify(sanitizedAfterState) : null, params.modification_reason || null, params.bulk_operation || false, params.affected_resources ? JSON.stringify(params.affected_resources) : null, params.batch_size || null, sanitizedDetails ? JSON.stringify(sanitizedDetails) : null, timestamp, true);
...
async function queryAuditTrail(params, db) {
    try {
        ...
        const countResult = db.prepare(countQuery).get(...queryParams.slice(0, -1));
...
export const trackAuditTrailTool = {
