...
function validateParams(params) {
    ...
    if (!params.file_hash)
        errors.push('file_hash is required');
...
async function uploadEvidence(params, db) {
    try {
        // Handle bulk upload
    ...
    `);
        const insertResult = insertStmt.run(evidenceId, params.profile_id, params.subcategory_id, sanitizedFileName, filePath, params.file_hash, params.file_size_bytes || null, params.evidence_type, sanitizedDescription, sanitizedUploadedBy, uploadDate, classification, retentionDays, params.metadata ? JSON.stringify(params.metadata) : null, true);
...
export const uploadEvidenceTool = {
