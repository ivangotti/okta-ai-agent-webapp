 ...
 */
export async function estimateImplementationCost(params) {
    ...
    try {
        ...
        for (const subcategoryId of params.subcategory_ids) {
            ...
            if (!estimate) {
                ...
                estimate = calculateSubcategoryCost(subcategoryId, params.organization_size, laborRate, framework);
                // Save to database
            ...
            subcategoryCosts.push({
                subcategory_id: subcategoryId,
                subcategory_name: (subcategory && 'name' in subcategory ? subcategory.name : subcategoryId),
        ...
        const costBreakdown = generateCostBreakdown(subcategoryCosts, params.organization_size, laborRate);
        ...
        const costOptimization = generateOptimizationRecommendations(subcategoryCosts, params.organization_size);
        return {
            success: true,
            organization_size: params.organization_size,
 ...
 */
function calculateRiskFactor(subcategoryId) {
    const functionId = subcategoryId.substring(0, 2);
 ...
 */
function calculateCostSummary(subcategoryCosts, params) {
 ...
 * Calculate cost by function
 */
function calculateCostByFunction(subcategoryCosts) {
 ...
 */
function generateCostBreakdown(subcategoryCosts, organizationSize, laborRate) {
    const breakdown = [];
    const totalCost = subcategoryCosts.reduce((sum, c) => sum + c.total_cost, 0);
    // Labor costs
 ...
 */
function calculateRiskAdjustedCosts(baselineCost) {
 ...
 */
function calculateOngoingCosts(subcategoryCosts, organizationSize) {
 ...
 */
function generateOptimizationRecommendations(subcategoryCosts, organizationSize) {
    ...
    if (organizationSize === 'small' || organizationSize === 'medium') {
        recommendations.potential_savings.push('Consider cloud-based security solutions to reduce upfront costs', 'Leverage managed security services for 24/7 monitoring', 'Use open-source tools where appropriate (with support contracts)');
    }
    // Phasing recommendations
    const highCostItems = subcategoryCosts.filter(c => c.total_cost > totalCost * 0.1);
    if (highCostItems.length > 3) {
        recommendations.phasing_recommendations.push(`Phase implementation of ${highCostItems.length} high-cost items over multiple quarters`, 'Prioritize quick wins and foundational controls in Phase 1', 'Defer advanced capabilities until core controls are operational');
    }
    ...
    for (const [functionId, count] of functionGroups) {
        if (count >= 3) {
            recommendations.bundling_opportunities.push(`Bundle ${count} ${getFunctionName(functionId)} subcategories for vendor discounts`, `Consider platform approach for ${getFunctionName(functionId)} to reduce integration costs`);
        }
    }
    // Add general recommendations
    if (subcategoryCosts.length > 10) {
        recommendations.bundling_opportunities.push('Negotiate enterprise agreement for comprehensive security platform', 'Consider multi-year contracts for better pricing');
    }
