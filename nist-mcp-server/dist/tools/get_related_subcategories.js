 ...
 */
export async function getRelatedSubcategories(params) {
    try {
        ...
        findRelatedSubcategories(framework, sourceSubcategory.element_identifier, params, relatedMap, visited, 1, params.depth);
        ...
        if (params.include_details) {
            for (const related of relatedSubcategories) {
                ...
                related.implementation_examples = framework.getImplementationExamples(related.subcategory.element_identifier);
            }
        }
 ...
 */
function findRelatedSubcategories(framework, currentId, params, relatedMap, visited, currentDepth, maxDepth) {
    ...
    for (const rel of relationships) {
        ...
        if (params.relationship_types && params.relationship_types.length > 0) {
            ...
            if (!params.relationship_types.includes(relType)) {
                continue;
            }
        }
        ...
        if (!existingRelated || existingRelated.relationship.distance > currentDepth) {
            ...
            if (currentDepth < maxDepth) {
                findRelatedSubcategories(framework, relatedId, params, relatedMap, visited, currentDepth + 1, maxDepth);
            }
        }
    }
}
 ...
 */
function inferRelationshipType(relationshipId) {
 ...
 */
function generateSummary(relatedSubcategories) {
    const summary = {
 ...
 */
function identifyClusters(relatedSubcategories) {
 ...
 */
function extractFunctionId(categoryId) {
 ...
 */
function extractCategoryId(subcategoryId) {
