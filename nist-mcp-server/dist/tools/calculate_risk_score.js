...
export async function calculateRiskScore(params) {
    ...
    try {
        ...
        const functionRisks = processFunctionRisks(riskData.function_risks, weights, framework);
        ...
        return createErrorResult(params.profile_id, error instanceof Error ? error.message : 'Unknown error occurred');
    }
}
/**
 * Process function risks with custom weights
 */
function processFunctionRisks(functionRisks, customWeights, framework) {
    const results = [];
    for (const risk of functionRisks) {
        // Get risk contributors (unimplemented subcategories)
        const riskContributors = getRiskContributors(risk.function_id, framework, risk.high_risk_count);
 ...
 */
function getRiskContributors(functionId, _framework, highRiskCount) {
    const contributors = [];
 ...
 */
function calculateOverallRisk(functionRisks, customWeights) {
    if (functionRisks.length === 0)
        return 0;
 ...
 */
function generateHeatMapData(profileId, db, _framework) {
    const heatMap = [];
 ...
 */
function calculateSubcategoryRisk(assessment) {
    let baseRisk = 0;
 ...
 */
function calculateRiskSummary(functionRisks, riskData) {
 ...
 */
function generateRiskRecommendations(functionRisks, overallRiskLevel, _riskSummary) {
    ...
    for (const func of criticalFunctions) {
        if (func.risk_contributors.length > 0) {
            criticalActions.push(`Address ${func.function_name}: ${func.risk_contributors[0]}`);
        }
    }
    // Risk mitigation strategies based on overall level
    if (overallRiskLevel === 'Critical') {
        riskMitigation.push('Implement emergency response procedures immediately', 'Conduct comprehensive security assessment', 'Engage external security expertise', 'Establish 24/7 monitoring capabilities');
    }
    else if (overallRiskLevel === 'High') {
        riskMitigation.push('Prioritize protection and detection controls', 'Enhance incident response capabilities', 'Implement security awareness training', 'Deploy additional monitoring tools');
    }
    else if (overallRiskLevel === 'Medium') {
        riskMitigation.push('Strengthen existing controls', 'Improve vulnerability management', 'Enhance access controls', 'Regular security assessments');
    }
    // Quick improvements for functions with moderate risk
    const moderateFunctions = functionRisks.filter(f => f.weighted_risk_score >= 30 && f.weighted_risk_score < 60);
    for (const func of moderateFunctions) {
        quickImprovements.push(`Quick win: Improve ${func.function_name} (${func.high_risk_count} high-risk areas)`);
    }
    return {
 ...
 * Create error result
 */
function createErrorResult(profileId, _message) {
    return {
        success: false,
