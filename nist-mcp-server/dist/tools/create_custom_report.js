...
function validateParams(params) {
    ...
    params.custom_sections?.forEach((section, index) => {
        if (!section.section_id)
            errors.push(`Section ${index + 1}: section_id is required`);
        if (!section.section_name)
            errors.push(`Section ${index + 1}: section_name is required`);
...
async function createCustomReport(params, db) {
    try {
        // Validate input
    ...
    `).all(params.profile_id);
        ...
        const totalDataPoints = generatedSections.reduce((sum, section) => sum + (section.metrics?.data_points || 0), 0);
...
async function generateCustomSections(sections, assessments, _db) {
    ...
    for (const sectionConfig of sections) {
        ...
        const sectionContent = generateSectionContent(sectionConfig.section_type, filteredData, sectionConfig.display_options, sectionConfig.custom_content);
...
function applyContentFilters(assessments, filters) {
    if (!filters)
        return assessments;
    let filteredData = [...assessments];
    // Filter by function IDs
    if (filters.function_ids && filters.function_ids.length > 0) {
        filteredData = filteredData.filter(a => filters.function_ids.includes(a.function_id));
    }
    // Filter by subcategory IDs
    if (filters.subcategory_ids && filters.subcategory_ids.length > 0) {
        filteredData = filteredData.filter(a => filters.subcategory_ids.includes(a.subcategory_id));
    }
    // Filter by maturity level range
    if (filters.maturity_level_min !== undefined) {
        filteredData = filteredData.filter(a => (a.maturity_score || 0) >= filters.maturity_level_min);
    }
    if (filters.maturity_level_max !== undefined) {
        filteredData = filteredData.filter(a => (a.maturity_score || 0) <= filters.maturity_level_max);
    }
    // Filter by implementation status
    if (filters.implementation_status && filters.implementation_status.length > 0) {
        filteredData = filteredData.filter(a => filters.implementation_status.includes(a.implementation_level));
    }
...
function generateSectionContent(sectionType, data, displayOptions, customContent) {
    switch (sectionType) {
        case 'summary':
...
function generateSummaryContent(data, _displayOptions) {
...
function generateMetricsContent(data, _displayOptions) {
    ...
    const metrics = Object.entries(functionBreakdown).map(([functionId, stats]) => ({
    ...
    return {
        ...
        overall_metrics: {
            ...
            highest_maturity_function: metrics.length > 0 ? metrics.reduce((max, curr) => curr.average_maturity > (max?.average_maturity || 0) ? curr : max, metrics[0]) : undefined,
            lowest_maturity_function: metrics.length > 0 ? metrics.reduce((min, curr) => curr.average_maturity < (min?.average_maturity || Infinity) ? curr : min, metrics[0]) : undefined
        }
    };
}
function generateAnalysisContent(data, displayOptions) {
...
function generateRecommendationsContent(data, _displayOptions) {
...
function generateChartsContent(data, _displayOptions) {
...
function generateTablesContent(data, displayOptions) {
...
// Helper functions
function generateRecommendedActions(control) {
    const actions = [];
...
function estimateImplementationEffort(control) {
...
function estimateImplementationTimeline(control) {
    const maturity = control.maturity_score || 0;
    if (maturity <= 1)
        return '2-3 months';
    if (maturity <= 2)
...
function assessBusinessImpact(control) {
    if (control.subcategory_id.includes('GV'))
        return 'High - Governance and oversight';
    if (control.subcategory_id.includes('ID'))
        return 'Medium - Asset and risk visibility';
    if (control.subcategory_id.includes('PR'))
        return 'High - Core protection capabilities';
    if (control.subcategory_id.includes('DE'))
        return 'Medium - Threat detection';
    if (control.subcategory_id.includes('RS'))
        return 'High - Incident response';
    if (control.subcategory_id.includes('RC'))
        return 'Medium - Recovery capabilities';
    return 'Medium - Security posture improvement';
}
function analyzeMaturityTrend(data) {
    ...
    if (avgMaturity >= 3.5)
        return 'Strong - Mature controls';
    if (avgMaturity >= 2.5)
        return 'Improving - Good progress';
    if (avgMaturity >= 1.5)
        return 'Developing - Needs attention';
    return 'Initial - Significant improvement needed';
}
function prepareChartData(data, _chartType, groupBy) {
...
function calculateSectionMetrics(content, sectionType) {
    switch (sectionType) {
        case 'charts':
            return {
...
function calculateReportStatistics(assessments, _sections) {
    const functions = Array.from(new Set(assessments.map(a => a.function_id)));
    const maturityScores = assessments.map(a => a.maturity_score || 0).filter(s => s > 0);
    const compliantControls = assessments.filter(a => a.implementation_level === 'fully_implemented' || a.implementation_level === 'largely_implemented').length;
    return {
...
function applyStyling(stylingOptions) {
    return {
...
function configureExportOptions(outputFormat, distributionSettings) {
...
function estimateGenerationTime(sectionCount, dataPoints) {
...
export const createCustomReportTool = {
