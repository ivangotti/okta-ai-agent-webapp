 ...
 */
export async function createImplementationPlan(params) {
    ...
    try {
        ...
        const subcategoryIds = gapItems.map((item) => item.subcategory_id);
        ...
        const sortedItems = sortItemsByStrategy(gapItems, params.prioritization_strategy, dependencyGraph);
        // Calculate phases
        const phases = calculatePhases(sortedItems, params.timeline_months, params.available_resources, params.phase_duration, dependencyGraph);
        // Calculate total effort
        const totalEffortHours = phases.reduce((sum, phase) => sum + phase.effort_hours, 0);
        ...
        const resourceAllocation = calculateResourceAllocation(phases, params.available_resources);
        // Generate recommendations
        const recommendations = generateRecommendations(phases, dependencyGraph, resourceAllocation, params);
        ...
        return {
            ...
            dependency_graph: dependencyGraph ? {
                nodes: subcategoryIds.map(id => ({
                    id,
                    name: gapItems.find((item) => item.subcategory_id === id)?.subcategory_name || id
                })),
 ...
 */
function sortItemsByStrategy(items, strategy, dependencyGraph) {
    const sorted = [...items];
    switch (strategy) {
        case 'risk_based':
        ...
        case 'dependencies_first':
            ...
            if (dependencyGraph?.topologicalOrder) {
                const orderMap = new Map(dependencyGraph.topologicalOrder.map((id, idx) => [id, idx]));
                sorted.sort((a, b) => {
 ...
 */
function calculateBalancedScore(item, dependencyGraph) {
    ...
    if (dependencyGraph) {
        const hasDependencies = dependencyGraph.dependencies.some((dep) => dep.from_id === item.subcategory_id);
        if (!hasDependencies) {
            score *= 1.2;
        }
    }
 ...
 */
function calculatePhases(items, timelineMonths, availableResources, phaseDuration, dependencyGraph) {
    const phases = [];
    ...
    for (const item of items) {
        ...
        if (dependencyGraph) {
            ...
            const deps = dependencyGraph.dependencies.filter((dep) => dep.from_id === item.subcategory_id);
            for (const dep of deps) {
                const depPhase = phases.findIndex(p => p.items.some(i => i.subcategory_id === dep.to_id));
                if (depPhase >= 0) {
 ...
 */
function getPhaseName(phaseNumber, totalPhases) {
    if (phaseNumber === 1) {
 ...
 */
function estimateItemEffort(item) {
    const baseEffort = {
        'not_implemented': 80,
 ...
 * Generate milestones
 */
function generateMilestones(phases, timelineMonths) {
    const milestones = [];
 ...
 * Calculate resource allocation
 */
function calculateResourceAllocation(phases, availableResources) {
 ...
 * Generate recommendations
 */
function generateRecommendations(phases, dependencyGraph, resourceAllocation, params) {
    ...
    if (dependencyGraph?.hasCycle) {
        recommendations.critical_dependencies.push('Circular dependencies detected - review and resolve before implementation');
    }
    if (dependencyGraph?.topologicalOrder) {
        const critical = dependencyGraph.topologicalOrder.slice(0, 3);
        recommendations.critical_dependencies.push(`Priority items: ${critical.join(', ')}`);
    }
    // Resource constraints
    const overloadedPhases = resourceAllocation.filter(a => a.workload_percentage > 80);
    if (overloadedPhases.length > 0) {
        recommendations.resource_constraints.push(`${overloadedPhases.length} phases have >80% resource utilization`);
        recommendations.resource_constraints.push('Consider adding resources or extending timeline');
    }
    // Risk factors
    if (params.timeline_months < 12) {
        recommendations.risk_factors.push('Aggressive timeline - ensure adequate change management');
    }
    if (phases.some(p => p.items.length > 20)) {
        recommendations.risk_factors.push('Large number of concurrent implementations - consider phased approach');
    }
    // Success factors
    recommendations.success_factors.push('Establish clear governance and oversight', 'Regular progress reviews and milestone tracking', 'Maintain flexibility to adjust based on learnings');
    if (params.prioritization_strategy === 'quick_wins') {
        recommendations.success_factors.push('Early wins will build momentum and stakeholder buy-in');
    }
