 ...
 */
export async function calculateMaturityTrend(params) {
    ...
    try {
        ...
        const { functionTrends, timeSeriesData } = processTrendData(trendData, params.aggregation_period);
        ...
        const velocityAnalysis = params.include_velocity_analysis
            ...
            : undefined;
        // Generate insights
        const insights = generateInsights(functionTrends, overallTrend, velocityAnalysis, projections);
 ...
 * Process and aggregate trend data
 */
function processTrendData(rawData, _aggregationPeriod) {
    ...
    for (const [functionId, dataPoints] of functionMap) {
        ...
        const daysDiff = Math.max(1, (new Date(current.date).getTime() - new Date(initial.date).getTime()) / (1000 * 60 * 60 * 24));
 ...
 * Calculate overall trend
 */
function calculateOverallTrend(functionTrends, _timeSeriesData) {
 ...
 */
function generateProjections(functionTrends) {
    const projections = [];
    for (const trend of functionTrends) {
        ...
        if (trend.volatility_score < 0.1)
            confidence = 'high';
        else if (trend.volatility_score > 0.3)
            confidence = 'low';
        projections.push({
 ...
 */
function analyzeVelocity(timeSeriesData, functionTrends) {
    ...
    let improvementTrend = 'constant';
    if (acceleration > 0.01)
        improvementTrend = 'accelerating';
    else if (acceleration < -0.01)
        improvementTrend = 'decelerating';
    return {
 ...
 */
function generateInsights(functionTrends, overallTrend, velocityAnalysis, projections) {
    const strengths = [];
    const concerns = [];
 ...
 * Create error result
 */
function createErrorResult(profileId, message) {
    return {
        success: false,
