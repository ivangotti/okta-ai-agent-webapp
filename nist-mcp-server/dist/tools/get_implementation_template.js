...
*/
export async function getImplementationTemplate(_db, framework, options) {
    try {
        ...
        const implementationSteps = generateImplementationSteps(subcategory, industry, orgSize);
...
function generateOverview(subcategory, category, func) {
...
function generateObjectives(subcategory) {
...
function generateImplementationSteps(subcategory, industry, orgSize) {
    const steps = [];
    // Step 1: Assessment
    steps.push({
        step_number: 1,
...
function getIndustryConsideration(industry, phase) {
...
function getEffortEstimate(orgSize, phase) {
    const estimates = {
        small: {
...
function generateExampleConfigurations(subcategory, _industry) {
    const examples = [];
...
function generateSuccessCriteria(subcategory) {
    const criteria = [
...
function generateMetrics(subcategory) {
    const metrics = [];
    const title = (subcategory.title || subcategory.text || subcategory.element_identifier || '').toLowerCase();
    // Add general metrics
    metrics.push({
...
function generateIndustryBestPractices(_subcategory, industry) {
    const practices = [
...
function generateIndustryChallenges(_subcategory, industry) {
    const challenges = [
...
function generateCommonPitfalls(_subcategory) {
    return [
...
function generateReferences(_subcategory, industry) {
    const references = [
        {
