        ...
        const result = db.transaction(() => {
            ...
            if (existingOrg) {
                ...
                db.createOrganization({
                    ...
                    current_tier: validatedParams.current_tier,
                    target_tier: validatedParams.target_tier
                });
 ...
 */
function initializeBaselineAssessments(db, profileId) {
    ...
    for (const subcategoryId of subcategoryPatterns) {
        ...
        const baseScore = baselineScores[functionId] || 1;
        assessments.push({
 ...
 */
function getImplementationLevel(score) {
    if (score === 0)
        return 'not_implemented';
    if (score <= 1)
        return 'partially_implemented';
    if (score <= 3)
        return 'largely_implemented';
