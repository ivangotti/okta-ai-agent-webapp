...
interface DecodedActClaim {
  sub?: string;
  act?: DecodedActClaim;
  [key: string]: unknown;
}

interface DecodedChainToken {
  sub?: string;
  aud?: string | string[];
  exp?: number;
  scope?: string;
...
/**
 * Metadata about the two-hop chain, for surfacing in tool results / the
 * webapp's Token Architecture viewer. Includes the raw ID-JAG and chained
 * access token strings alongside their decoded claims - unlike the PAM
 * exchange's vaulted secret/password, these are short-lived OAuth bearer
 * tokens (same tier as the idJagToken/mcpAccessToken already shown in the
 * clear elsewhere in that viewer), not standing credentials, so there's no
 * need to gate them behind a DEBUG reveal.
 */
export interface AgentA2AChainMeta {
  agentAClientId: string;
  agentBClientId: string;
  hop1: {
    ...
    expiresIn?: number;
    /** Agent A's client_assertion JWT for this hop. */
    clientAssertion: string;
    /** The raw ID-JAG issued by this hop. */
    idJag: string;
    /** Decoded claims of the ID-JAG. */
    idJagClaims: DecodedChainToken;
  };
  hop1Redeem: {
    tokenEndpoint: string;
    issuedTokenType?: string;
  ...
  /** Raw chained access token (T3) - same value as the top-level accessToken result. */
  chainedAccessToken: string;
  /** Decoded claims of the final access token (T3) - the chain-of-custody proof. */
  chainedAccessTokenClaims: DecodedChainToken;
  fetchedAt: string;
}

export interface AgentA2AChainResult {
  accessToken: string;
  meta: AgentA2AChainMeta;
}
...
let lastRevealableChainTokens: { idJag: string; accessToken: string; fetchedAt: string } | null = null;

export function getLastRevealableA2AChain(): { idJag: string; accessToken: string; fetchedAt: string } | null {
 ...
 */
export async function getAgentBChainedAccessToken(userIdToken: string): Promise<AgentA2AChainResult> {
  ...
  const hop1Data = (await hop1Response.json().catch(() => ({}))) as TokenExchangeResponse;
  ...
  const idJagClaims = (jwt.decode(idJag) as DecodedChainToken | null) || {};
  ...
  const hop1RedeemData = (await hop1RedeemResponse.json().catch(() => ({}))) as TokenExchangeResponse;
  ...
  const decoded = (jwt.decode(chainedAccessToken) as DecodedChainToken | null) || {};
