...
function buildSearchExpression(params: SearchUsersParams): string {
 ...
 */
export async function searchUsers(apiToken: string, params: SearchUsersParams): Promise<OktaUserSummary[]> {
  ...
  const users = (await response.json()) as OktaUserRecord[];
